...
class Widget_Image extends Widget_Base {
	...
	protected function _content_template() {
		...
		<# if ( settings.image.url ) {
			...
			var getAttachmentCaption = function( url ) {
				if ( ! url ) {
					return '';
				}
				var a=url.split('/').pop().split('#')[0].split('?')[0].split('.')[0];
				return a;
			}
			...
			var getCaption = function() {
				...
				return 'custom' === settings.caption_source ? settings.caption : getAttachmentCaption( settings.image.url );
			}
			...
			if ( link_url ) {
					#><a class="elementor-clickable" data-elementor-open-lightbox="{{ settings.open_lightbox }}" href="{{ link_url }}"><#
						...
						#><img alt="{{ settings.image_alt }}" title="{{ settings.image_title }}" src="{{ image_url }}" class="{{ imgClass }}"  /><#
