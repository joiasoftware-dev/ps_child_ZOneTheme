...
class Widget_Toggle extends Widget_Base {
	...
	public function get_title() {
		return PrestaHelper::__( 'Toggle', 'elementor' );
	}
	...
	protected function _register_controls() {
		$this->start_controls_section(
			'section_toggle',
			[
				'label' => PrestaHelper::__( 'Toggle', 'elementor' ),
			]
		);
		...
		$repeater->add_control(
			...
			[
				...
				'default' => PrestaHelper::__( 'Toggle Title', 'elementor' ),
				'label_block' => true,
			]
		);
		...
		$repeater->add_control(
			...
			[
				...
				'default' => PrestaHelper::__( 'Toggle Content', 'elementor' ),
				'show_label' => false,
			]
		);
		...
		$this->add_control(
			'tabs',
			[
				'label' => PrestaHelper::__( 'Toggle Items', 'elementor' ),
				'type' => Controls_Manager::REPEATER,
				'fields' => $repeater->get_controls(),
				'default' => [
					[
						'tab_title' => PrestaHelper::__( 'Toggle #1', 'elementor' ),
					...
					[
						'tab_title' => PrestaHelper::__( 'Toggle #2', 'elementor' ),
						'tab_content' => PrestaHelper::__( 'Click edit button to change this text. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Ut elit tellus, luctus nec ullamcorper mattis, pulvinar dapibus leo.', 'elementor' ),
					],
				],
		...
		$this->add_control(
			...
			[
				...
				'default' => [
					'value' => 'fas fa-caret' . ( PrestaHelper::is_rtl() ? '-left' : '-right' ),
					'library' => 'fa-solid',
				],
			]
		);
	...
	protected function render() {
		...
		if ( ! isset( $settings['icon'] ) && ! Icons_Manager::is_migration_allowed() ) {
			$settings['icon'] = 'fa fa-caret' . ( PrestaHelper::is_rtl() ? '-left' : '-right' );
		...
		<div class="elementor-toggle" role="tablist">
			...
			foreach ( $settings['tabs'] as $index => $item ) :
				...
				<div class="elementor-toggle-item">
					<<?php echo PrestaHelper::esc_html( $settings['title_html_tag'] ); ?> <?php echo $this->get_render_attribute_string( $tab_title_setting_key ); ?>>
						<?php if ( $has_icon ) : ?>
						<span class="elementor-toggle-icon elementor-toggle-icon-<?php echo PrestaHelper::esc_attr( $settings['icon_align'] ); ?>" aria-hidden="true">
							<?php
							if ( $is_new || $migrated ) { ?>
							...
							<?php } else { ?>
								<i class="elementor-toggle-icon-closed <?php echo PrestaHelper::esc_attr( $settings['icon'] ); ?>"></i>
								<i class="elementor-toggle-icon-opened <?php echo PrestaHelper::esc_attr( $settings['icon_active'] ); ?>"></i>
							<?php } ?>
						</span>
						<?php endif; ?>
						<a href=""><?php echo $item['tab_title']; ?></a>
					</<?php echo PrestaHelper::esc_html( $settings['title_html_tag'] ); ?>>
